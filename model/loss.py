...
class SpETLoss(Loss):
    '''
    Loss for datasets without relations.
    '''
...
class SpRTLoss(Loss):
    def __init__(self, rel_criterion, model, optimizer, scheduler, max_grad_norm):
        self._rel_criterion = rel_criterion
        self._model = model
        self._optimizer = optimizer
        self._scheduler = scheduler
    ...
    def compute(self, rel_logits, rel_types, rel_sample_mask):
        # relation loss
        rel_logits = rel_logits.view(-1, rel_logits.shape[-1])
        rel_types = rel_types.view(-1, rel_types.shape[-1])
        rel_sample_mask = rel_sample_mask.view(-1).float()
