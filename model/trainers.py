...
class SpERTTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader, 
        ...
        print("@@@@@ PARAMETER COUNT @@@@@@@@@@@ ---> {}".format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
...
class SpETTrainer(BaseTrainer):
    ...
    def eval(self, eval_path: str, train_path: str, predicted_entities_path: str, 
        ...
        # evaluate
        ner_eval, predictions = self._eval(model, input_reader.get_dataset(dataset_label), input_reader)
        self._logger.info("Logged in: %s" % self._log_path)
        self._sampler.join()
        print("Saving predicted dataset of {} sentences...".format(len(predictions)))
        util.save_json(predictions, self._log_path, "predictions.json")
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(sampler, total=total, desc='Train epoch %s' % epoch):
            ...
            # compute loss and optimize parameters
            batch_loss = compute_loss.compute(0, 0, entity_logits, batch.entity_types, 0, batch.entity_sample_masks)
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: JsonInputReader,
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(sampler, total=total, desc='Evaluate epoch %s' % epoch):
                ...
                evaluator.eval_batch_entities(entity_clf, batch, return_conversions=True)
